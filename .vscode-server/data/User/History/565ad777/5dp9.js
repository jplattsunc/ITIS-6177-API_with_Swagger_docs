 ...
 * /product-types:
 *   get:
 *     summary: Get all distinct product types from the foods table
 *     responses:
 *       200:
...
app.get('/product-types', async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        //query to retrieve distinct product types (FOOD_TYPE)
        const rows = await conn.query('SELECT DISTINCT FOOD_TYPE FROM foods');
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    } finally {
 ...
 */
app.post('/customers', [
    //ensure CUST_CODE is between 1 and 6 characters and required.
    body('CUST_CODE').isLength({ min: 1, max: 6 }).withMessage('CUST_CODE must be between 1 and 6 characters'),
    //trim whitespace and ensure CUST_NAME is not empty.
...
app.patch('/customers/:id', [
    //validate that the id is between 1 and 6 characters.
    param('id').isLength({ min: 1, max: 6 }).withMessage('ID must be between 1 and 6 characters'),
    //trim and check if CUST_NAME is provided and not empty.
    ...
    //trim the CUST_NAME value if provided.
...
app.post('/orders', [
    ...
    body('ORD_DESCRIPTION').optional().trim() //trim any whitespace.
], async (req, res) => {
    const errors = validationResult(req);
...
app.post('/products', [
    ...
    body('FOOD_NAME').trim().notEmpty().withMessage('FOOD_NAME is required'),
