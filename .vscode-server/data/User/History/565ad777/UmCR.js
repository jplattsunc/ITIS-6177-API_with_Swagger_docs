...
app.get('/customers/:id', async (req, res) => {
    const customerId = req.params.id; // No extra transformation here
    let conn;
    try {
        conn = await pool.getConnection();
 ...
 */
app.post('/customers', [
    // Sanitization & Validation: Check length for CUST_CODE and trim CUST_NAME.
    ...
    // Transformation: CUST_NAME is trimmed.
...
app.post('/orders', [
    ...
    body('ORD_DESCRIPTION').optional().trim() // Trim whitespace if provided.
], async (req, res) => {
    const errors = validationResult(req);
